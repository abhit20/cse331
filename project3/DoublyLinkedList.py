...
class DLL:
    ...
    def count_unique(self):
        """
        Counts the number of unique characters in the list
        :return:
        """
        quick_sort(self, self.head, self.tail, self.size, 20)
        dll = DLL([])
        node = self.get_head()
        sum = 0
        lst = []
        ...
        for i in range(len(lst)-1):
            if lst[i] != lst[i+1]:
                new_node = DLLNode(lst[i])
                dll.set_head(new_node)
                dll.set_tail(new_node)
                if lst.count(lst[i]) != 1:
                    new_node = DLLNode(lst.count(lst[i]))

        new_node = DLLNode(lst[-1])
        dll.set_tail(new_node)
        if lst.count(lst[-1]) != 1:
            new_node = DLLNode(lst.count(lst[-1]))
            dll.set_tail(new_node)
